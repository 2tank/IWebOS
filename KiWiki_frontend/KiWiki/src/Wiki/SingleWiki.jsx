...
function SingleWiki({ item }) {
    ...
    const clickWiki = () => {
        ...
        navigate('/wiki/'+`${item.name}`+'/entries/' , {
            state: { id: item._id },
          });
        };
    ...
    <div onClick={clickWiki} className="flex w-full flex-col bg-white shadow-md rounded-lg p-6 m-4 hover:shadow-lg transition-shadow duration-300">
